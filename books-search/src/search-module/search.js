    ...
    for (let i = idx; i < summaries.length; i++) {
        ...
        if (num > 0) {
            ...
            if (constructedData[subStr].length > MAX_LENGTH) {
                constructedData[subStr] = constructedData[subStr].slice(0, AVG_LENGTH+1);
            }
            ...
            if (dataMap.length > 1) {
                while (tempTitleList.length < k) {
                    ...
                    if (equalValFound) {
                        ...
                        for (let j = 0; j < wL; j++) {
                            if (dataMap[j][indexArr[j]][0] < dataMap[minIdx][indexArr[minIdx]][0]) {
                                minIdx = j;
                            }
                        }
...
async function writePreprocessedData() {
    try {
        if (constructedData !== '') {
            ...
            let writingComplete = await fsPromises.writeFile('./preprocessedData.json', jsonString);
